...
When launching graphical QEMU from an agent, detach the launcher session itself (for example, `setsid -f python3 util/run.py ... >build-debug/qemu-window-launch.log 2>&1`); a plain `nohup ... &` can be reaped with the shell and make a successful boot look like a QEMU startup failure. For windowed runs, pass a build-local `--monitor` socket and verify `info status` reports `VM status: running`; an empty launcher log is not evidence that QEMU failed. A stale monitor socket is only removable after verifying it belongs to the selected build directory and no matching QEMU process is running.

`python3 util/run.py ... q` refreshes `/boot` through `fstool` and installs the rootless BIOS GRUB loader into the build-local `image/disk.img`; if that build-local image is missing, it creates and formats a new exFAT image first. Existing images are not reformatted. A manually created image from `disk.py create` must be formatted before it can be used. Validate p1 with `python3 util/disk.py mount --build-dir build-debug`; use `q --iso` when an ISO boot is specifically required. The separately generated `bin/system/root.img` is formatted by `make_root_image.py` as the prepared FAT root image. Normal service startup must never format either medium.
