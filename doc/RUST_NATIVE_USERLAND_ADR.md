# ADR：零 mlibc 依赖的 Rust Native 用户态
...
- 本文对应基线：2026-09-12 当前实现；本文记录已采纳的架构、ABI、迁移边界和验证要求。

## 0. 决策结论

1. `ProcessBuilder` 的 deferred-start 不承诺恢复已发送的 MOVE resource。bootstrap
   消息成功提交后，resource ownership 按 [Capability 与 Invocation ADR](OBJECT_CALL_ADR.md)
   转移；后续启动失败只报告失败并清理 child 侧状态，不伪造回滚 authority。
2. runtime startup frame 是 argv/env 的唯一事实源。`ProcessBuilder` 只转发已构造的
   bootstrap 数据，不复制或维护第二份环境快照；startup frame 的提交是原子边界。
3. compiler TLS 继续以 [Rust Native TLS ABI ADR](RUST_NATIVE_TLS_ADR.md) 为 ABI 权威；
   本 ADR 的 Rust `std` 支持仅限第 6 节明确列出的已实现 PAL 与 `Unsupported` 边界，
   不把未提供的 capability surface 伪装成完整 Rust `std`。
...
- Phase 4 交付 capability-native Rust `std` 的最小实现切片：锁定 Rust fork/stage1 的 source tree、
  `build-std` 编译链、argv/env snapshot、time、native thread/parking/TLS destructor、`id`/`exit` 和
  kernel-backed random/hash keys。ProcessControl/SignalReceiver、File/Directory/Stream capability 尚未
  落地；filesystem/network/Command/stdio 等 surface 明确保持 `Unsupported`，不得用 host Linux fallback
  模拟它们。

### 3.3 可验收的结果

在干净构建和 `python3 util/run.py --build-dir build-debug q --iso -n` 启动中：
...
- `current_dir`、`set_current_dir`、`current_exe`、`temp_dir`、home directory 与 `PATH` 查找依赖
  文件/namespace ADR，Phase 4 必须返回 `Unsupported` 或保持未发布。纯 `OsString` 操作可用，但不能
  由此暗示路径可以被解析。
...
当前 NaOS `std` 使用 kernel `getrandom` syscall 作为最小随机源：它只接受 kernel 提供的硬件随机值，
不使用 `/dev/urandom`、host syscall、当前 process 的 `AT_RANDOM` seed，或基于地址/时间的弱随机数。
后续如果需要策略隔离、quota 或服务化随机源，再引入 `RandomSource` typed capability；当前 PAL 不得
自行发现全局服务。

- 当前 `std::sys::random::fill_bytes`/`hashmap_random_keys` 通过 `naos-runtime` 调用该 syscall；一次请求
  精确填满目标 buffer，kernel 在无法提供硬件随机值时返回明确错误。
- 未来 `RandomSource::fill(length) -> bytes` 的 canonical response 必须精确包含请求长度的 CSPRNG
  bytes，单次长度不超过 channel bound，并由 client 分块处理任意 `std` 请求。服务端不得接受 client
  raw pointer；请求、response、quota 与 cancellation 走 NaoIDL/Invocation 的正常 ownership 规则。
- 随机能力只表示读取 CSPRNG，不代表管理 entropy pool 或其他 system authority。无论是当前 syscall
  还是未来 service，在 cryptographic source 不可用、quota 拒绝、peer close、revocation 或 transport
  unknown 时都必须明确失败；绝不可用可预测回退填充结果。随机 bytes 既用于 hashmap keys，也可供
  `std` 的其他内部需求使用。当前 kernel 实现由内核统一执行 RDSEED/RDRAND 选择；用户态不直接执行
  硬件指令，也不把它们作为 PAL 的失败回退。
- Rust `std` 的内部 `fill_bytes` 无可恢复错误返回时，PAL 仅能写出不泄露 entropy 的有限诊断并按
  ...
  程序不因缺少随机相关的未来 capability 失去启动能力；启用 NaOS `std` 的程序在当前 syscall
  失败时按上述 abort 语义处理。
...
3. `fork`、`pre_exec`、Unix `CommandExt`、process-group shell compatibility 与把 open capability
   ...
   File/Directory/Stream 的语义，必须等用户态文件系统 ADR 与 stdio adapter 通过 review 后另行启用；当前
   ProcessBuilder 不是这些 API 的暗中替代品。
4. 现有 spawn frame 中的 diagnostic path 只是当前 loader 输入，不是长期授权边界。用户态文件系统
   executable 演进为 MemoryObject capability 时，`Executable` wrapper 保持抽象而不暴露
...
| --- | --- | --- |
| `std::fs`、`File`、metadata、directory iteration、path cwd | Unsupported | [用户态文件系统 ADR](USERSPACE_FILESYSTEM_ADR.md) 完成 capability File/Directory、MObj exec 与 Rust `io::Error` mapping review。 |
...
| `std` random/hash keys | 已实现最小切片 | `std` PAL → `naos-runtime` → kernel `getrandom`；错误时 abort；无弱随机 fallback | `/dev/urandom`、host syscall、clock/address fallback |
...
| 随机源不可用时继续运行 | kernel `getrandom`/未来 RandomSource failure 对 `std` 是 abort path，绝不退回时间、地址、零或旧 bytes；no_std 不受该 capability 影响。 |
...
和受控 filesystem/network/Command Unsupported；该切片仍不承诺 dynamic TLS、`dlopen`、服务化
RandomSource、SignalReceiver 或 capability Child。
